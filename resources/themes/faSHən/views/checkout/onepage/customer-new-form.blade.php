...
@endphp
    ...
    @if($gst_general_buyer_gst)
    <div :class="`col-12 form-field ${errors.has('address-form.billing[gstin]') ? 'has-error' : ''}`">
            <label for="billing[gstin]">
                {{ __('GST Number') }}
            </label>
            ...
            <input
                ...
                id="billing[gstin]"
                name="billing[gstin]"
                v-model="address.billing.gstin"
                @change="validateForm('address-form')"
                data-vv-as="&quot;{{ __('shop::app.checkout.onepage.gstin') }}&quot;" />

            <span class="control-error" v-if="errors.has('address-form.billing[company_name]')">
                @{{ errors.first('address-form.billing[gstin]') }}
            </span>
        </div>
    @endif
